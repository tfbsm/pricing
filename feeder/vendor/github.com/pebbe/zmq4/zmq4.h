...
int zmq4_bind(void* socket, char const* endpoint);
int zmq4_close(void* socket);
int zmq4_connect(void* socket, char const* endpoint);
...
int zmq4_disconnect(void* socket, char const* endpoint);
int zmq4_getsockopt(void* socket, int option_name, void* option_value, size_t* option_len);
char const* zmq4_msg_gets(zmq_msg_t* message, char const* property);
int zmq4_msg_recv(zmq_msg_t* msg, void* socket, int flags);
int zmq4_poll(zmq_pollitem_t* items, int nitems, long timeout);
int zmq4_proxy(void const* frontend, void const* backend, void const* capture);
int zmq4_proxy_steerable(void const* frontend, void const* backend, void const* capture,
                         void const* control);
int zmq4_send(void* socket, void* buf, size_t len, int flags);
int zmq4_setsockopt(void* socket, int option_name, void const* option_value, size_t option_len);
void* zmq4_socket(void* context, int type);
int zmq4_socket_monitor(void* socket, char* endpoint, int events);
int zmq4_unbind(void* socket, char const* endpoint);
