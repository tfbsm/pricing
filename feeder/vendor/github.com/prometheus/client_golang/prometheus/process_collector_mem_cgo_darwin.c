...
int get_memory_info(unsigned long long* rss, unsigned long long* vsize) {
    ...
    error = task_info(mach_task_self(), MACH_TASK_BASIC_INFO, (task_info_t)&info, &info_count);
    ...
    *vsize = info.virtual_size;

    {
        ...
        error = mach_vm_region(mach_task_self(), &address, &size, VM_REGION_BASIC_INFO_64,
